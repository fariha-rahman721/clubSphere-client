...
const MembershipCard = ({ onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const membershipData = {
      ...
      status: "active",       // For free join, mark as active
      paymentId: null,
      joinedAt: new Date(),
      expiresAt: null,
    };
    ...
    try {
      // Send membership data to backend
      const response = await axios.post(
        "http://localhost:3000/joinClubs",
        membershipData
      );
      ...
      // Optional: call parent onSubmit
      if (onSubmit) onSubmit(membershipData);

      // Reset form
      setFormData({ userEmail: "", clubId: "" });
    } catch (error) {
      console.error(error);
      toast.error("Failed to join the club. Try again!");
    }
  };
